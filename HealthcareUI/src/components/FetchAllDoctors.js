...
function FetchAllDoctors(props) {
  ...
  useEffect(() => {
    const fetchDoctors = async () => {
      try {
        ...
        if (error.message === 'Request failed with status: 400') {
          setError('Welcome again! Below are the active doctors');
        } else {
  ...
  return (
    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: '20px' }}>
      {doctors.map((doctor) => (
        <NewPatient key={doctor.doctorId} parse_doctor={doctor} onConsultClick={handleConsultClicktwo} passstatus={props.clickstatus} />
      ))}
    </Box>
  );
}
